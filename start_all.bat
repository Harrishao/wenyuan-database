...
echo.
echo [2/3] 启动后端服务...
start "文渊后端服务" cmd /k "cd backend && if not exist .venv (python -m venv .venv && .venv\Scripts\python.exe -m pip install -e ".[dev]" && Copy-Item ..\.env.example .env && .venv\Scripts\alembic.exe upgrade head) && .venv\Scripts\uvicorn.exe app.main:app --reload --port 4396"
