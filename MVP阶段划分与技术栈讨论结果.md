...
**状态**：程序实现、算法分层样例、真实数据库端到端验收及生产构建已完成，待用户验收。
...
- 管理员登录与角色鉴权；
- 用户列表、启用、禁用和用量查看；
- 模板、章节、提示词、模型参数和检索参数的版本化配置；
- 敏感词增删、批量导入以及上传/生成后的扫描；
- 管理员关键操作审计、按时间和操作类型搜索；
- 文件类型、大小、路径和内容安全校验；
- 大模型调用超时、限流、重试和错误提示；
- 解析、向量化、生成任务的幂等处理；
- 核心服务单元测试与上传—生成—查重的集成测试；
- 深浅主题及关键页面的空状态、错误状态和加载状态。

**范围控制**：

- 管理端优先保证功能完整，不单独建设一套技术架构；复用同一 React 应用和 FastAPI 服务，通过路由与权限区分；
- 敏感内容命中后先进入“标记/限制使用”状态，不在 MVP 中自动物理删除用户文件；
- `API Key` 仅通过服务端环境变量或密钥管理注入，不允许前端读取或由普通管理员查看明文。
...
| 缓存/队列 | Redis + RQ/Dramatiq 在阶段 4 引入 | 解析和生成任务需要可重试、可恢复；早期可先用数据库任务表和单 worker |

FastAPI 自带后台任务适合较轻、同进程任务；文献解析、Embedding 和长报告生成属于较重任务，最终应放入独立 worker。阶段 1 可先实现数据库任务表与单 worker，避免一开始同时调试过多基础设施。
