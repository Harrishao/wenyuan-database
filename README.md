...
- 报告装配台采用活动栏、主侧栏、编辑区和 AI 副侧栏组成的 IDE 式布局；
- 每份报告支持多个可重命名、可删除的连续对话；
- 对话记录按账号隔离并持久化到数据库；
- 每次提问携带当前完整报告、当前章节检索证据和按模型预设保留的最近完整对话轮次；
- 超出轮数限制时从最早的完整问答轮次开始丢弃，不拆散单轮消息；
- AI 回复使用 SSE 流式输出并逐段显示；
- 回答只能引用本次检索结果中能够映射到真实片段的编号；
- 普通对话、局部润色和学术助手分别绑定独立的提示词变体；
- 普通问答不会修改报告，局部润色只有确认后才建立新版本。

### 硬盘配额与 Credits

- 学生账号默认硬盘配额为 50 MiB，管理员账号不设上限；
- 原始文献、聊天正文、报告当前内容及历史版本计入硬盘配额；
- 可重新生成的解析文本、向量、索引和缓存不计入用户配额；
- 学生账号每个自然月刷新 300 Credits，不结转，管理员不受限制；
- 管理员可以单独调整学生的硬盘配额、月度 Credits，并进行本期一次性增发；
- 外部 LLM 的报告生成、章节重试、局部润色和聊天均记录消耗，离线能力免费；
- LLM 预设分别配置每百万输入 Tokens 和输出 Tokens 的 Credits；
- 渠道返回 usage 时使用真实数值，否则按预设 tokenizer 兼容策略估算并明确标记。
...
- LLM 预设配置上下文窗口、最大输出、保留对话轮数、usage 模式及输入/输出费率；
- 提示词按报告生成、普通对话、局部润色和学术助手分类，并为每种角色或风格保存完整 messages；
- 提示词支持 system/user/assistant 消息、排序、停用、宏变量和按变体发布；
- 敏感词分组、启停、增删修改和批量导入；
...
模型由管理员为整个平台统一配置。每个提示词功能/变体可以独立启用一个版本，普通用户只能选择管理员已经启用的润色风格、助手角色和工作模式。
...
- 提示词预设：功能、角色/风格、消息编排、宏变量和启用状态；
- Embedding 预设：本地或第三方模型、维度和重建向量；
- 敏感词：分组、词项和启停；
- 报告模板：模板信息、章节、参数和版本发布；
- 内容审核：敏感词命中、审核意见和处置；
- 校园公告：草稿、发布、置顶和下线；
- 用户与用量：账号状态、资源数量、硬盘配额、月度 Credits、一次性增发和密码重置；
- 审计记录：条件筛选和 CSV 导出。
...
- 不接入公网论文库，不宣称提供权威查重率；
- 系统暂不压缩超大报告；对话上下文只按照 LLM 预设的完整轮数上限裁剪历史消息；
